...
export async function GET(
  ...
  const url = new URL(
    `https://api.us-east.tinybird.co/v0/pipes/v1_os.json?workId=${workspaceId}`
  );

  try {
    const response = await fetch(url.toString(), {
      headers: {
        Authorization: `Bearer ${process.env.TINYBIRD_API_OS_KEY as string}`,
      },
    });
    ...
    return NextResponse.json<any>({ error }, { status: 500 });
  }
}
